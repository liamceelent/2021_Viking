...
@app.route("/")
def home():
    if session.get("name") is not None:  # checking if user is logged in or not
        pass
    else:
        session["name"] = "Guest"
...
def login_post():
    ...
    if request.method == "GET":  # did the browser ask to see the page
        ...
        if form.validate_on_submit():
            ...
            name_check = models.User.query.filter_by(
                name=name).first()  # seeing if name is in databse
            if name_check is not None:
                salt = name_check.salt
                key = name_check.key
                new_key = hashlib.pbkdf2_hmac("sha256", password.encode(
                    "utf-8"), salt, 100000)  # hashing password to check
                if key == new_key:
                    session["name"] = name
                    ...
                    return render_template(
                        "login.html", status=status, form=form)
            else:
                status = "Wrong user name or password."
...
def sign_up_post():
    ...
    if request.method == "GET":  # did the browser ask to see the page
        ...
        name_check = models.User.query.filter_by(
            name=name).first()  # seeing if name or email is in databse
            ...
            if email_check is None:
                ...
                if email_check is not None:
                    ...
                    with smtplib.SMTP("smtp.gmail.com", 587) as smtp:  # how i send email
                        ...
                        smtp.starttls()
                        smtp.ehlo()
                        # email and password for the email i set up also at top
                        # of code
...
def map():
    if request.method == "GET":
        current_user = session.get("name")
        period = 1
        locations = models.Location_Faction.query.filter_by(
            period=period).all()
        location = []
...
# getting what comment goes where
    ...
    if request.method == "GET":
        ...
        if request.method == "POST" and "search" in request.form:
            ...
            if request.method == "POST" and "last" in request.form:
                ...
                questions = models.Question.query.filter(
                    models.Question.question.like(search)).all()
            ...
            if request.method == "POST" and "recent" in request.form:
                ...
                search = "%{}%".format(search)
                questions = models.Question.query.filter(
                    models.Question.question.like(search)).order_by(
                    models.Question.id.desc()).all()
        ...
        if request.method == "POST" and "recent" in request.form:
            ...
            questions = models.Question.query.order_by(
                models.Question.id.desc()) .all()  # seeing recent post using ids decesnding
            return render_template(
                "question.html",
                form=form,
                page_title="history",
...
def famous():
    if request.method == "GET":
        ...
        if request.method == "POST" and "rating_low" in request.form:
            print("hi")
            vikings = models.Famous_Viking.query.order_by(
                models.Famous_Viking.rating.asc()) .all()  # seeing recent post using ids decesnding
            return render_template(
                "famous.html",
                page_title="famous",
                user=current_user,
        ...
        if request.method == "POST" and "rating_high" in request.form:
            ...
            vikings = models.Famous_Viking.query.order_by(
                models.Famous_Viking.rating.desc()) .all()  # seeing recent post using ids decesnding
            return render_template(
                "famous.html",
                page_title="famous",
                user=current_user,
...
def famous_click():
    ...
    if not stats:
        return redirect(url_for("famous"))
    else:
        return render_template(
            "famous_click.html",
            page_title="famous",
